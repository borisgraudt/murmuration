...
echo "0⃣ Переустанавливаю бинарник mur (чтобы использовать последнюю версию)..."
...
echo "1⃣ Останавливаю все существующие узлы..."
pkill -f "mur start" || true
sleep 1

# Удалить файл, который может мешать (ВАЖНО: этот файл переопределяет --port!)
echo "2⃣ Удаляю ~/.murmuration_api_port (если существует)..."
echo "     Этот файл может переопределять флаг --port!"
rm -f ~/.murmuration_api_port

# Запустить узлы
echo ""
echo "3⃣ Запускаю узлы..."
echo ""
...
echo "4⃣ Определяю реальные API порты из логов..."
echo ""
...
echo "5⃣ Проверяю статус каждого узла..."
echo ""

echo "   Узел 1:"
mur status --port "$API_PORT_1" 2>&1 | head -5 || echo "    Не удалось подключиться"
echo ""

echo "   Узел 2:"
mur status --port "$API_PORT_2" 2>&1 | head -5 || echo "    Не удалось подключиться"
echo ""

echo "   Узел 3:"
mur status --port "$API_PORT_3" 2>&1 | head -5 || echo "    Не удалось подключиться"
echo ""
