...
Raw data: `benchmark.csv`, `learning_curve.csv`. 10 seeds per cell, 2 000 messages
per run unless stated, 95% CI by normal approximation.
...
n=50, p=0.20, 40 000 messages, 5 seeds:

| `s` | ucb1 | **agnostic_limit (ceiling)** | **q_routing** | oracle |
|---|---|---|---|---|
| 0.0 (uniform) | 10.0% ± 0.7 | **13.0% ± 0.6** | 7.6% ± 0.4 | 76.7% ± 2.8 |
| 0.8 | 9.2% ± 0.7 | **11.5% ± 0.6** | 10.4% ± 0.7 | 76.9% ± 3.2 |
| 1.2 | 8.2% ± 1.0 | 9.7% ± 1.0 | **14.3% ± 2.6** | 76.7% ± 3.0 |
...
* **s = 0.0** — Q-routing loses badly to the ceiling (7.6 vs 13.0). It cannot
  gather enough samples per (node, destination) pair to converge.
* **s = 0.8** — roughly level (10.4 vs 11.5), CIs overlap.
* **s = 1.2** — Q-routing beats the ceiling with **disjoint CIs** (11.7 vs 10.7),
  a ~47% relative improvement, and beats `ucb1` by ~74%.

Two further observations:

1. Every destination-agnostic arm *degrades* as traffic concentrates
   (ceiling 13.0 → 11.5 → 9.7; `ucb1` 10.0 → 9.2 → 8.2), because concentration
   removes the averaging that a destination-blind policy implicitly relies on.
   Q-routing moves the opposite way (7.6 → 10.4 → 14.3). **The two curves cross
   because they depend on concentration with opposite sign.**
