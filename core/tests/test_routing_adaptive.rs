...
async fn test_route_history_tracking() {
    ...
    router.record_route_success("peer1", Duration::from_millis(50)).await;
    router.record_route_success("peer1", Duration::from_millis(60)).await;
    router.record_route_failure("peer2").await;
    router.record_route_success("peer2", Duration::from_millis(100)).await;
...
async fn test_adaptive_scoring() {
    ...
    let mut good_peer = PeerInfo::new(
        "good_peer".to_string(),
...
async fn test_adaptive_learning_with_history() {
    ...
    for _ in 0..5 {
        router.record_route_success("test_peer", Duration::from_millis(50)).await;
    }
...
async fn test_route_success_rate_impact() {
    ...
    assert_eq!(selected[0], "peer1", "Peer with better success rate should be selected");
}
