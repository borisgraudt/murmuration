...
async fn test_content_request_timeout() {
    ...
    let result = timeout(
        ...
        node.fetch_content("ely://nonexistent_node/some/path", Duration::from_millis(500))
    ).await;
...
async fn test_invalid_url_handling() {
    ...
    config.listen_addr = "127.0.0.1:0".parse().unwrap();
    config.api_addr = Some("127.0.0.1:0".parse().unwrap());
    config.data_dir = Some(PathBuf::from(format!(".ely/test-invalid-url-{}", std::process::id())));
    ...
    for url in invalid_urls {
        ...
        assert!(result.is_err() || result.unwrap().is_none(), 
            "URL '{}' should return error or None, not panic", url);
    }
}
...
async fn test_concurrent_content_requests() {
    ...
    for i in 0..5 {
        ...
        let handle = tokio::spawn(async move {
            ...
            node_clone.fetch_content(&url, Duration::from_millis(100)).await
        });
        handles.push(handle);
    }
    ...
    assert!(result.is_ok(), "Concurrent requests should complete without panic");
}
...
async fn test_very_long_url() {
    ...
    let result = node.fetch_content(&long_url, Duration::from_millis(100)).await;
