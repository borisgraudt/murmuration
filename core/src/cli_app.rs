    ...
    match command.as_str() {
        ...
        "chat" => {
            ...
            if remaining.is_empty() {
                eprintln!(
                    "{}",
                    format!(
                        "Usage: {} chat <peer_id|broadcast> [--port <api_port>]",
                        bin
                    )
                    .yellow()
                );
        ...
        "send" => {
            ...
            if remaining.len() < 2 {
                eprintln!(
                    "{}",
                    format!(
                        "Usage: {} send <peer_id> <message> [--port <api_port>]",
                        bin
                    )
                    .yellow()
                );
        ...
        "broadcast" => {
            ...
            if remaining.is_empty() {
                eprintln!(
                    "{}",
                    format!("Usage: {} broadcast <message> [--port <api_port>]", bin).yellow()
                );
                return Ok(());
            }
        ...
        "ping" => {
            ...
            if remaining.is_empty() {
                eprintln!(
                    "{}",
                    format!(
                        "Usage: {} ping <peer_id> [timeout_ms] [--port <api_port>]",
                        bin
                    )
                    .yellow()
                );
            ...
            let timeout_ms = remaining
                ...
                .unwrap_or(1500);
            ping(peer_id, timeout_ms, port)?;
        }
        "peers" => {
            let (port, _) = extract_port_flag(&args[2..]);
            list_peers(port)?;
        }
        "status" => {
            let (port, _) = extract_port_flag(&args[2..]);
        ...
        "publish" => {
            ...
            if remaining.len() < 2 {
                eprintln!(
                    "{}",
                    format!(
                        "Usage: {} publish <path> <content|@file|@dir> [--port <api_port>]",
                        bin
                    )
                    .yellow()
                );
            ...
            let path = remaining[0].clone();
            let content_arg = remaining[1..].join(" ");

            // Check if it's a directory
            if let Some(dir_path) = content_arg.strip_prefix('@') {
                if let Ok(metadata) = std::fs::metadata(dir_path) {
                    if metadata.is_dir() {
                        publish_directory(&path, dir_path, port)?;
                    } else {
                        publish(path, content_arg, port)?;
                    }
                } else {
                    publish(path, content_arg, port)?;
                }
...
/// Extract --port or -p flag from args, returns (port_option, remaining_args)
fn extract_port_flag(args: &[String]) -> (Option<u16>, Vec<String>) {
    let mut port = None;
    ...
    while i < args.len() {
        if args[i] == "--port" || args[i] == "-p" {
            ...
            // Invalid --port, keep it in remaining for error handling
...
fn get_api_port(port_override: Option<u16>) -> u16 {
    // Priority 1: Command line flag --port or -p
...
fn send_message(
    ...
    let mut stream = TcpStream::connect(format!("127.0.0.1:{}", api_port)).map_err(|e| {
        anyhow::anyhow!(
            "Failed to connect to API on port {}: {}. Make sure the node is running on this port.",
            api_port,
            e
        )
    })?;
...
fn chat(target: String, port_override: Option<u16>) -> anyhow::Result<()> {
    let _api_port = port_override.unwrap_or_else(|| get_api_port(None));
    if let Some(port) = port_override {
        eprintln!(
            "{} Connecting to API on port {} {}",
            "✓".green(),
    ...
    for line in stdin.lock().lines() {
        ...
        if target == "broadcast" {
            let _ = send_message(None, msg.to_string(), port_override);
        } else {
            let _ = send_message(
