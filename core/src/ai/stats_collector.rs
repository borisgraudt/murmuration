...
impl PeerStats {
    ...
    pub fn reliability_score(&self) -> f32 {
        ...
        (self.trust_score * (1.0 - self.packet_loss) * (1.0 - error_rate)).max(0.0).min(1.0)
    }
}
...
impl StatsCollector {
    ...
    pub async fn record_latency(&self, peer_id: &str, latency: Duration) {
        ...
        let peer_stats = stats.entry(peer_id.to_string())
    ...
    pub async fn calculate_score(&self, peer_id: &str) -> f32 {
        ...
        if let Some(peer_stats) = stats.get(peer_id) {
            let latency_score = peer_stats.latency
                .map(|lat| {
    ...
    pub async fn cleanup_stale(&self, timeout: Duration) {
        ...
        stats.retain(|_, peer_stats| {
            now.duration_since(peer_stats.last_seen) < timeout
        });
    }
}
...
mod tests {
    ...
    async fn test_stats_collector() {
        ...
        // Record some stats
        collector.record_latency("peer1", Duration::from_millis(50)).await;
    ...
    async fn test_score_calculation() {
        ...
        collector.record_latency("peer1", Duration::from_millis(10)).await;
        collector.update_uptime("peer1", Duration::from_secs(3600)).await;
